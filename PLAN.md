...
- Release policy: `master` を base branch とし、SemVer、internal `workspace:` dependency、Changeset version PR、protected `release` environment での manual publish、unpublish 禁止の forward-fix/patch rollback を固定した。CI は `governance:check` を実行し、PR template は lifecycle/evidence/Changeset/migration review を要求する。公開配布の package boundary と npm Trusted Publishing/OIDC の追加判断は ADR-008 に固定した。
- Known limitations: 実 Product の production usage、NVDA/VoiceOver の manual evidence、実 Figma publish、GitHub public visibility、npm scope ownership、release environment reviewer/secret は外部運用で補完する。codemod は登録した Component JSX prop だけを変換し、Product 固有の data/API migration は手動対応とする。placeholder app の build output warning は既存の Phase 0 境界として残る。
- Public distribution preparation: `@depo-ui/react` の runtime dependency closure を public package boundary とし、`@depo-ui/react/css`、package allowlist、packed consumer check、Changeset 初回リリース意図、OIDC/bootstrap gate、provenance policy を実装した。実 registry publish は安全のため実行していない。
